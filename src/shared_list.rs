...
impl<T: Clone> SharedList<T> {
    ...
    /// # #[macro_use] extern crate im_lists;
    /// # use im_lists::shared_list;
    /// let list = shared_list![1usize, 2, 3];
    /// let vec = vec![4, 5, 6];
    /// assert_eq!(list.extend(vec), shared_list![1, 2, 3, 4, 5, 6])
    /// ```
    pub fn extend(self, iter: impl IntoIterator<Item = T>) -> Self {
        SharedList(self.0.extend(iter))
    }
}
...
impl<T: Clone> FromIterator<T> for SharedList<T> {
    fn from_iter<I: IntoIterator<Item = T>>(iter: I) -> Self {
        SharedList(iter.into_iter().collect())
    }
}

impl<T: Clone> FromIterator<SharedList<T>> for SharedList<T> {
    fn from_iter<I: IntoIterator<Item = SharedList<T>>>(iter: I) -> Self {
        SharedList(iter.into_iter().map(|x| x.0).collect())
    }
}

impl<T: Clone> From<Vec<T>> for SharedList<T> {
    fn from(vec: Vec<T>) -> Self {
        SharedList(vec.into_iter().collect())
    }
}
...
pub struct IterRef<'a, T: Clone>(IterWrapper<'a, T, ArcConstructor, ArcConstructor>);
...
pub struct Iter<T: Clone>(ConsumingWrapper<T, ArcConstructor, ArcConstructor>);
